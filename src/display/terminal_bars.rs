            ...
            Ok(bands) => {
                    ...
                    && count > 0
                {
                    ...
                    for i in 0..NUM_BANDS {
                        ...
                        avg[i] = to_db_display(band_acc[i] / count as f32);

                        // Exponential smoothing
                        displayed[i] = 0.8 * displayed[i] + 0.2 * avg[i];
...
fn draw_bars(
    ...
    for i in 0..NUM_BANDS {
        ...
        let scaled = (value / 80.0).clamp(0.0, 1.0)* BAR_WIDTH as f32;

        let full_blocks = scaled.floor() as usize;

        let partial = ((scaled - full_blocks as f32) * 8.0).round() as usize;

        let mut cells =vec![" ".to_string(); BAR_WIDTH];
        ...
        if partial > 0 && full_blocks < BAR_WIDTH {
            cells[full_blocks] = BLOCKS[partial.min(8)].to_string();
        }
        // }

        let peak_pos = ((peaks[i] / 80.0)* BAR_WIDTH as f32).clamp(0.0, (BAR_WIDTH - 1) as f32) as usize;
        ...
        println!("{:<10} | {} {:5.1}", BAND_LABELS[i], bar, value);
    }
